...
namespace DeveloperTest.Model
{
    public abstract class ConnectionBase : IMailConnection
    {
        ...
        public static IMailConnection CreateConnection(ConnectionDetails details, TaskScheduler scheduler)
        {
            switch (details.ConnectionType)
            {
                case ConnectionType.IMAP:
                    return new ImapConnection(details, scheduler);
                case ConnectionType.POP3:
                    return new Pop3Connection(details, scheduler);
            }
            return null;
        }
            ...
            try
            {
                switch (details.EncryptionType)
                {
                    ...
                    case EncryptionType.STARTTLS:
                        ...
                        // -> I therefore don't know how I can test this
                        client.Connect(details.Servername, details.Port);
                        if (isImapConnection)
        ...
        protected void RunMailBodyDownloadTask(MailInfo mailInfo, CancellationToken token)
        {
